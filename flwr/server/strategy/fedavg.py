...
class FedAvg(Strategy):
    ...
    def aggregate_fit(
        ...
        '''
        li = []
        for i in client_weight_dict.keys() : 
            if i in [0,1,2,3,4] : 
                li.append(client_weight_dict[i])

        log(INFO,
            f"Load the weights for each local model... length of the weight is {len(li)}")
        '''
        ...
        '''
        c_norm = [torch.norm(torch.from_numpy(weights[0][-1]), dim=0) for weights in weights_results]
        
        c_norm_median = np.median(c_norm) 

        li = [] 
        for norm in c_norm :
            if norm > c_norm_median :
                log(INFO, f"Abnormal client's norm ={norm}")
                log(INFO, f"Abnormal client = {print(c_norm.index(norm))}")
                li.append(c_norm.index(norm))
        
        weights_results = [weights_results[i] for i in li]
        ...
        log(INFO,
            f"Load the weights for each local model... length of the weight is {len(weights_results)}")
